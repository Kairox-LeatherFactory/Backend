"""
================================================================================
RETIRED — app/modules/drawers/schemas.py
================================================================================
COMMENTED OUT 2026-09-20 (Hamthan's call: "if you want to delete then just
comment out it"). Kept rather than deleted so the shape is on hand if any of it
is ever wanted back.

WHY IT IS GONE

Physical drawers are not tracked. There were 200 of them; a single style
releases a hundred-plus garments, so the pool ran out, and allocating them by
hand was too complicated for the DM to actually do. The drawer was a location,
and a location is not what anyone needed to know — they needed to know what the
STORE is holding for a garment and what it is still waiting for.

So the store became seven states on the PIECE itself — `store_state`,
`leather_in`, `lining_in`, `accessories_in`, `store_entered_at` — and the scan
went from three (drawer, employee, piece) to two (employee, piece). The piece
barcode already says which garment it is; the drawer only ever said where it
happened to be sitting. See docs/KAIROX_PRODUCTION-EVENT_SYSTEM_GUIDE.md s14
and CLAUDE.md s8.

WHAT REPLACED IT
    /api/v1/drawers/*   ->  /api/v1/store/*        (app/modules/store/)
    DrawerService._needs_lining  ->  StoreService.needs_lining

STATUS
    - This router was never registered: main.py:106 and :322 are both commented.
    - Nothing in application code imports this file.
    - The drawer TABLES remain, unwritten, because they hold historical rows.
    - drawers/service.py is still imported by ~8 test files and is therefore
      left live for now; it has no remaining caller in application code.
================================================================================
"""
